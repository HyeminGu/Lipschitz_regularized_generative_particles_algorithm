...
# Training loop -------------------------------------------------
p.mb_size_Q  = min(p.mb_size_Q, p.N_samples_Q) 
p.mb_size_P  = min(p.mb_size_P, p.N_samples_P) 
from util.train_NN import decay_learning_rate, update_NN
from util.evaluate_metric import calc_fid, calc_ke

def train(mb_Y_, X_, mb_Y_label, X_label, p, NN_par, NN_par_m=None):
    lr_NN = tf.Variable(p.lr_NN, trainable=False)
    lr_P = tf.Variable(p.lr_P, trainable=False)
    lr_Ps = []
    
    trajectories, vectorfields, divergences, mobilities, KE_Ps, FIDs = [], [], [], [], [], []

    P = tf.Variable(mb_Y_, dtype=tf.float32) # variable
    if p.N_conditions >1:
        P_label = tf.constant(mb_Y_label , dtype=tf.float32)
    else:
        P_label = None
    ...
    for it in range(1, p.epochs+1): 
        random_idx = np.random.choice(X_.shape[0], p.mb_size_Q, replace=False)
        Q = tf.constant(X_[random_idx], dtype=tf.float32) # constant
        if p.N_conditions >1:
            Q_label = tf.constant(X_label[random_idx] , dtype=tf.float32) 
        else:
            Q_label = None
    ...
    return trajectories, vectorfields, divergences, mobilities, KE_Ps, FIDs, lr_Ps
        
# Run -------------------------------------------------------
import time 
t0 = time.time()
divergences, KE_Ps = [0]*p.epochs, [0]*p.epochs
FIDs = [0]*int(p.epochs/p.save_iter)
lr_Ps = [0]*p.epochs

for i in range(int(p.N_samples_P/p.mb_size_P)): 
    mb_Y_ = Y_[i*p.mb_size_P:(i+1)*p.mb_size_P]
    if type(Y_label) != np.ndarray:
        mb_Y_label = None
    else:
        mb_Y_label = Y_label[i*p.mb_size_P:(i+1)*p.mb_size_P]
    print("{}th mini batch".format(i+1))
    if p.mobility == 'ML':
        trajectories_mb, vectorfields_mb, divergences_mb, mobilities_mb, KE_Ps_mb, FIDs_mb, lr_Ps_mb = train(mb_Y_,X_, mb_Y_label, X_label, p, NN_par, NN_par_m)
    else:
        trajectories_mb, vectorfields_mb, divergences_mb, mobilities_mb, KE_Ps_mb, FIDs_mb, lr_Ps_mb = train(mb_Y_,X_,  mb_Y_label, X_label, p, NN_par)
    
    
    KE_Ps = [KE_Ps[j] + KE_Ps_mb[j] for j in range(p.epochs)]
    divergences = [divergences[j] + p.mb_size_P*divergences_mb[j] for j in range(p.epochs)]
    print(len(FIDs_mb), len(FIDs))
    if len(FIDs_mb) >0:
        FIDs = [FIDs[j] + p.mb_size_P*FIDs_mb[j] for j in range(len(FIDs_mb))]
    else:
        FIDs = []
        
    if len(lr_Ps_mb) >0:
        lr_Ps = [lr_Ps[j] + p.mb_size_P*lr_Ps_mb[j] for j in range(len(lr_Ps_mb))]
    else:
        lr_Ps = []
    
    if i == 0:
        trajectories = trajectories_mb
        vectorfields = vectorfields_mb
        mobilities = mobilities_mb
        
    else:
        for j in range(int(p.epochs/p.save_iter)):
            trajectories[j] = np.concatenate((trajectories[j], trajectories_mb[j]), axis=0)
            if vectorfields != []:
                vectorfields[j] = np.concatenate((vectorfields[j], vectorfields_mb[j]), axis=0)
            if mobilities != []:
                mobilities[j] = np.concatenate((mobilities[j], mobilities_mb[j]), axis=0)

divergences = [divergences[j]/p.N_samples_P for j in range(p.epochs)]
FIDs = [FID/p.N_samples_P for FID in FIDs]
lr_Ps = [lr_P/p.N_samples_P for lr_P in lr_Ps]
...
result = {'trajectories': trajectories, 'vectorfields': vectorfields, 'divergences': divergences, 'mobilities': mobilities, 'KE_Ps': KE_Ps, 'FIDs': FIDs}
